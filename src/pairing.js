...
// Track whether socket is ready for pairing
let socketReady = false;
let currentSock = null;

export function setSocketReady(sock) {
  currentSock = sock;
  socketReady = true;
}
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .card{background:rgba(10,10,30,0.85);border:1px solid rgba(0,255,200,0.18);border-radius:20px;padding:48px 40px;max-width:460px;width:90%;backdrop-filter:blur(20px);box-shadow:0 0 60px rgba(0,255,200,0.08);position:relative;z-index:1;}
    .logo{text-align:center;margin-bottom:32px;}
    .logo h1{font-family:'Orbitron',monospace;font-weight:900;font-size:2.4rem;background:linear-gradient(135deg,#00ffc8 0%,#8c00ff 50%,#00b4ff 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:4px;}
    ...
    input{width:100%;padding:14px 18px;background:rgba(0,255,200,0.05);border:1px solid rgba(0,255,200,0.2);border-radius:10px;color:#e0e0ff;font-family:'Rajdhani',sans-serif;font-size:1rem;outline:none;transition:border-color .3s;}
    ...
    button{width:100%;margin-top:24px;padding:15px;background:linear-gradient(135deg,#00ffc8,#8c00ff);border:none;border-radius:10px;color:#fff;font-family:'Orbitron',monospace;font-weight:700;font-size:0.9rem;letter-spacing:2px;text-transform:uppercase;cursor:pointer;transition:opacity .3s,transform .2s;box-shadow:0 4px 30px rgba(0,255,200,0.25);}
...
<body>
  ...
  <script>
    ...
    async function generateCode() {
      ...
      btn.disabled  = true;
      btn.innerHTML = '<span class="spinner"></span>Connecting to WhatsApp... (5s)';
      try {
  ...
  // ── Pairing API — mirrors V4's exact approach with 3s delay ──
  app.post('/pair', async (req, res) => {
    try {
      ...
      // ── CRITICAL: wait 3 seconds just like V4 does ──────────
      // This gives the Baileys WebSocket time to reach
      // "connecting" state before requestPairingCode is called.
  ...
  app.listen(port, () => {
    ...
    console.log(chalk.cyan('  ║') + chalk.green('  Open URL → enter number → get code') + chalk.cyan('  ║'));
    console.log(chalk.cyan('  ╚══════════════════════════════════════╝\n'));
  });
                }
