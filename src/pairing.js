...
// Import live sock reference from index.js
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .result-label{font-size:.72rem;letter-spacing:2px;color:rgba(0,255,200,.65);text-transform:uppercase;margin-bottom:10px;}
    .code{
    ...
    .err{
      ...
      border-radius:8px;color:#ff8080;font-size:.82rem;
    }
...
<body>
  ...
  <div class="card">
    ...
    <p class="hint">Include country code &mdash; no + or spaces</p>
    ...
    <div class="steps">
      ...
      3. Enter the 8-digit code above within 60 seconds
    </div>
  </div>
  ...
  <script>
    ...
    async function generate() {
      ...
      if (!phone || phone.length < 7) {
        err.textContent = '⚠ Enter a valid number with country code (e.g. 254712345678)';
        err.classList.add('show');
        return;
      }

      btn.disabled    = true;
      btn.innerHTML   = '<span class="spinner"></span>Please wait (5s)...';
  ...
  // ── POST /pair — the only endpoint ───────────────────────────
  // This mirrors V4's pairing exactly:
  ...
  app.post('/pair', async (req, res) => {
    try {
      ...
      if (!sock) return res.status(503).json({ error: 'Bot socket not ready. Wait a few seconds and try again.' });

      // Check not already registered
      if (sock.authState?.creds?.registered) {
        return res.status(400).json({ error: 'Bot is already paired! Restart to re-pair.' });
      }

      console.log(chalk.yellow(`  🔑 Pairing requested for: ${number}`));

      // ── THE EXACT V4 PATTERN ──────────────────────────────────
      // V4 line 170: await new Promise(r => setTimeout(r, 3000));
      // V4 line 171: const code = await sock.requestPairingCode(phone);
      await new Promise(r => setTimeout(r, 3000));
      const code = await sock.requestPairingCode(number);

      console.log(chalk.bold.green(`\n  ╔══════════════════════════════════════╗`));
      console.log(chalk.bold.green(`  ║   🔑 PAIRING CODE: `) + chalk.bold.cyan(code) + chalk.bold.green(`         ║`));
      console.log(chalk.bold.green(`  ╚══════════════════════════════════════╝\n`));
