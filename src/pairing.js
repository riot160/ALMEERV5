...
let _sock        = null;
let _cachedCode  = null;   // stores code generated by env/terminal option

export function updateSock(sock)  { _sock = sock; }
export function setCachedCode(c)  { _cachedCode = c; }
...
export function startPairingServer(port) {
  ...
  app.post('/pair', async (req, res) => {
    try {
      ...
      // ── KEY FIX ───────────────────────────────────────────────
      // If a code was already generated by env/terminal option,
      // return that SAME code instead of calling requestPairingCode
      // again. Calling it twice corrupts the pairing handshake.
      if (_cachedCode) {
        log.pair(`Returning cached code for: ${number}`);
        return res.json({ code: _cachedCode });
      }

      // No cached code — generate fresh (web-only flow)
