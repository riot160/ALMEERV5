...
// ── Socket reference updated by index.js on every (re)connect ────────────────
let _sock = null;
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    body{
      font-family:'Rajdhani',sans-serif;background:#060610;color:#e0e0ff;
      min-height:100vh;display:flex;align-items:center;justify-content:center;
    }
    body::before{
      content:'';position:fixed;top:-50%;left:-50%;width:200%;height:200%;
      background:
        radial-gradient(ellipse at 30% 20%,rgba(0,255,200,.07) 0%,transparent 50%),
        radial-gradient(ellipse at 70% 80%,rgba(140,0,255,.09) 0%,transparent 50%);
      pointer-events:none;
    }
    .grid{
    ...
    .card{
      background:rgba(10,10,30,.92);border:1px solid rgba(0,255,200,.18);
      border-radius:20px;padding:44px 40px;max-width:460px;width:90%;
      backdrop-filter:blur(20px);
      box-shadow:0 0 60px rgba(0,255,200,.08),0 0 100px rgba(140,0,255,.05);
      position:relative;z-index:1;
    }
    h1{
      font-family:'Orbitron',monospace;font-weight:900;font-size:2.2rem;
    ...
    .sub{color:rgba(0,255,200,.6);font-size:.82rem;letter-spacing:2px;
    ...
    input{
      width:100%;padding:14px 16px;background:rgba(0,255,200,.05);
      border:1px solid rgba(0,255,200,.2);border-radius:10px;color:#e0e0ff;
      font-size:1rem;outline:none;font-family:'Rajdhani',sans-serif;
    ...
    .btn{
      ...
      background:linear-gradient(135deg,#00ffc8,#8c00ff);
      border:none;border-radius:10px;color:#fff;
      font-family:'Orbitron',monospace;font-weight:700;
      font-size:.88rem;letter-spacing:2px;text-transform:uppercase;
      cursor:pointer;box-shadow:0 4px 30px rgba(0,255,200,.25);
    ...
    .result{
      display:none;margin-top:26px;padding:22px 20px;
      background:rgba(0,255,200,.05);border:1px solid rgba(0,255,200,.25);
      border-radius:14px;text-align:center;
    }
    .result.show{display:block;}
    .rlabel{font-size:.72rem;letter-spacing:2px;color:rgba(0,255,200,.65);
    ...
    .copy{
      margin-top:14px;padding:9px 22px;background:rgba(0,255,200,.1);
      border:1px solid rgba(0,255,200,.28);border-radius:8px;
      color:rgba(0,255,200,.9);font-family:'Rajdhani',sans-serif;
      font-size:.85rem;letter-spacing:2px;cursor:pointer;
      transition:background .2s;width:auto;box-shadow:none;
    }
    ...
    .err{
      ...
      border-radius:8px;color:#ff8080;font-size:.85rem;line-height:1.5;
    }
    .err.show{display:block;}
    .steps{margin-top:24px;font-size:.75rem;color:rgba(255,255,255,.3);
  ...
  <script>
    ...
    async function generate() {
      ...
      try {
        ...
        if (!r.ok || data.error) throw new Error(data.error || 'Server error — check Railway logs');
...
export function startPairingServer(port) {
  ...
  // ── POST /pair — THE only pairing endpoint ────────────────────
  app.post('/pair', async (req, res) => {
    try {
      ...
      if (!_sock) {
        return res.status(503).json({ error: 'Bot is starting up — wait 5 seconds and try again' });
      }

      if (_sock.authState?.creds?.registered) {
        return res.status(400).json({ error: 'Bot is already paired! To re-pair: use .clearsession command or delete the session/ folder and redeploy.' });
      }

      console.log(chalk.yellow(`\n  🔑 Pairing requested for: ${number}`));

      // ── Exact V4 pairing pattern: 3s wait then request code ───
      await new Promise(r => setTimeout(r, 3000));
      const code = await _sock.requestPairingCode(number);

      console.log(chalk.bold.green(`  ╔══════════════════════════════════════╗`));
      console.log(chalk.bold.green(`  ║  🔑 CODE: `) + chalk.bold.cyan(code) + chalk.bold.green(`                      ║`));
      console.log(chalk.bold.green(`  ╚══════════════════════════════════════╝\n`));
  ...
  app.listen(port, () => {
    ...
    console.log(chalk.cyan('  ║') + chalk.yellow('  🌐 PAIRING SERVER RUNNING              ') + chalk.cyan('║'));
    console.log(chalk.cyan('  ║') + chalk.white('  Visit your Railway URL to pair        ') + chalk.cyan('║'));
    console.log(chalk.cyan('  ╚════════════════════════════════════════╝\n'));
  });
}
