...
// No import from index.js — avoids circular dependency
// index.js imports pairing.js, so pairing.js must NOT import index.js

let _sock = null;
export function updateSock(sock) { _sock = sock; }

// Simple local logger for pairing server
const log = {
...
export function startPairingServer(port) {
  ...
  app.post('/pair', async (req, res) => {
    try {
      ...
      if (!_sock)
        return res.status(503).json({ error: 'Bot is starting up — wait a few seconds and try again.' });
      if (_sock.authState?.creds?.registered)
        return res.status(400).json({ error: 'Bot is already paired!' });
