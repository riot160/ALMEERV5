...
async function statusHandler(sock, msg) {
  ...
  if (settings.autoViewStatus) {
    ...
    try {
      ...
      } catch (e2) { log.warn(`View failed: ${e2.message}`); }
    }
  }
  ...
  if (settings.autoReactStatus) {
    ...
    } catch (e) { log.warn(`React failed: ${e.message}`); }
  }
...
async function startBot() {
  ...
  if (!pairingServerStarted) {
    ...
    log.pair('Pairing server started — visit your URL to pair');
  }

  // ── Pairing — web form only (same as working version) ─────────
  // requestPairingCode is called ONLY from /pair POST route in pairing.js
  // It is NOT called here at startup — this was the bug
  if (!sock.authState.creds.registered) {
    log.warn('Not yet paired — visit your Railway/Pterodactyl URL to pair');
  } else {
  ...
  sock.ev.on('connection.update', ({ connection, lastDisconnect }) => {
    if (connection === 'connecting') log.conn('Connecting to WhatsApp...');
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        log.cmd2(isOwner ? 'OWNER' : m.sender.split('@')[0], `.${m.command}`, m.jid.split('@')[0]);
