...
export const log = {
  ...
  msg:    (from, body) => console.log(
    ...
    chalk.dim(' → ') +
    chalk.white(String(body).slice(0, 60) + (String(body).length > 60 ? '...' : ''))
  ),
...
async function startBot() {
  ...
  // NO browser override — same as working V4
  ...
  if (!sock.authState.creds.registered) {
    ...
    // OPTION 2: OWNER_NUMBER in env — auto-generate at startup
    const envNumber = process.env.OWNER_NUMBER?.replace(/[^0-9]/g, '');
    if (envNumber) {
      log.pair(`OWNER_NUMBER in env → auto-generating code for ${envNumber}...`);
      try {
        await new Promise(r => setTimeout(r, 3000));
        const code = await sock.requestPairingCode(envNumber);
        config.OWNER = envNumber;
        // ── CRITICAL: cache so web form returns THIS code, not a new one
        setCachedCode(code);
        ...
        log.pair('Same code is also visible at your web URL');
      } catch (err) {
        log.error(`Auto-pair failed: ${err.message}`);
      }

    } else {
      // OPTION 3: No env — ask in terminal
      log.pair('No OWNER_NUMBER in env — enter number below or use web URL');
      ...
      if (termNumber && termNumber.length >= 7) {
        ...
        try {
          ...
          console.log(chalk.bold.hex('#39ff14')('  ╚══════════════════════════════════════════════╝'));
          console.log(chalk.dim('  → WhatsApp → Linked Devices → Link with phone number\n'));
          log.pair('Same code visible at your web URL');
        } catch (err) {
          log.error(`Terminal pair failed: ${err.message}`);
          log.info('Use the web URL to pair manually instead');
        }
      } else {
  ...
  // ── Main message handler — notify AND append ──────────────────
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const raw of messages) {
      try {
        ...
        log.event(`Anti-link deleted msg from ${sender.split('@')[0]}`);
      } catch (_) {}
    }
  });
}
