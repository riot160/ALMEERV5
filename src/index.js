...
// ── These run only ONCE at process start ─────────────────────────────────────
...
async function startBot() {
  ...
  // ── Load plugins ONCE only ────────────────────────────────────
  if (!pluginsLoaded) {
    await loadPlugins();
    pluginsLoaded = true;
  }

  // ── Create socket — exact same config as V4 ───────────────────
  ...
  // ── Always give pairing server the LATEST socket ──────────────
  updateSock(sock);

  bindStore(sock);

  // ── Start pairing web server ONCE ────────────────────────────
  ...
  // ── connection.update — mirrors V4 logic ─────────────────────
  sock.ev.on('connection.update', ({ connection, lastDisconnect }) => {
      ...
      if (config.OWNER) {
        ...
        sock.sendMessage(ownerJid, {
          text:
            ...
            `_Type ${config.PREFIX}menu to see commands_ 🚀`,
        }).catch(() => {});
      }
    }
    ...
    if (connection === 'close') {
      ...
      // ── V4 EXACT LOGIC: loggedOut = exit, else retry ──────────
      if (statusCode === DisconnectReason.loggedOut) {
        console.log(chalk.red('\n  ❌ Logged out of WhatsApp.'));
        console.log(chalk.yellow('  Clearing session and exiting — Railway will restart.\n'));
        fs.emptyDirSync(config.SESSION_PATH);
        process.exit(0);   // Railway auto-restarts on exit, like V4's process.exit(1)
      } else {
        const reason = DisconnectReason[statusCode] || statusCode;
        console.log(chalk.yellow(`  🔄 Disconnected (${reason}) — reconnecting in 5s...`));
  ...
  // ── Anti-delete (correct event, same as V4) ───────────────────
