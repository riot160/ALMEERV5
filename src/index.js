...
// ── Single shared socket reference ───────────────────────────────────────────
...
async function startBot() {
  ...
  // ── Build socket — mirrors V4 exactly ────────────────────────
  sock = makeWASocket({
    version,
    logger,
    auth: {
  ...
  // The server holds a reference to always get the LATEST sock
  // so after reconnects it still works.
  if (!pairingServerStarted) {
    pairingServerStarted = true;
    startPairingServer(config.PORT);
    console.log(chalk.yellow(`  ⚠️  Not yet paired.`));
    console.log(chalk.cyan(`  🌐 Open your Railway URL to pair your device.\n`));
  }
  ...
  // ── Store all messages (required for anti-delete) ─────────────
  ...
  // ── Anti-delete via messages.delete (correct Baileys event) ───
