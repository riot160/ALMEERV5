...
import { startPairingServer, updateSock, setCachedCode } from './pairing.js';
...
export const log = {
  ...
  cmd:    (from, cmd, jid) => console.log(
    chalk.bold.magenta(`  [CMD]    `) +
    chalk.hex('#ff9de2')(from) + chalk.dim(' ➤ ') +
    chalk.magentaBright(`.${cmd}`) + chalk.dim(` | ${jid}`)
  ),
  ...
  group:  (m) => console.log(chalk.bold.hex('#ff6ec7')(`  [GROUP]  `) + chalk.hex('#ff9de2')(m)),
};
...
function printBanner() {
  ...
  console.log(
    chalk.bold.hex('#00ffc8')('   ⚡  ALMEERV5') +
    chalk.hex('#bf00ff')(' WhatsApp Bot  ') +
    chalk.hex('#39ff14')('v5.0.0')
  );
  console.log(chalk.dim('   github.com/SIDER44  |  ALMEER Brand'));
  console.log(chalk.hex('#00ffc8')('  ' + '═'.repeat(50)) + '\n');
}

// ── Terminal prompt — non-blocking, runs AFTER all handlers are registered ────
function startTerminalPairing(sock) {
  // Only prompt if stdin is an interactive terminal (Pterodactyl console)
  // On Railway/cloud with no TTY, skip silently — web URL is available
  if (!process.stdin.isTTY) {
    log.pair('No TTY detected — use your web URL to pair');
    return;
  }

  const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
  console.log(chalk.bold.hex('#bf00ff')('\n  ╔══════════════════════════════════════════╗'));
  console.log(chalk.bold.hex('#bf00ff')('  ║      📱 ENTER YOUR WHATSAPP NUMBER       ║'));
  console.log(chalk.bold.hex('#bf00ff')('  ╚══════════════════════════════════════════╝'));
  console.log(chalk.dim('  Include country code — no + or spaces'));
  console.log(chalk.dim('  Example: 254712345678'));
  console.log(chalk.dim('  Press Enter to skip and use the web URL instead\n'));

  rl.question(chalk.bold.hex('#00ffc8')('  ➤ Phone number: '), async (input) => {
    rl.close();
    const number = input.trim().replace(/[^0-9]/g, '');
    if (!number || number.length < 7) {
      log.warn('No number entered — use your web URL to pair');
      return;
    }
    log.pair(`Requesting code for: ${number}...`);
    try {
      ...
      console.log(chalk.dim('  → Same code also available at your web URL\n'));
    } catch (err) {
      log.error(`Terminal pair failed: ${err.message}`);
      log.warn('Use your web URL to pair instead');
    }
  });
}
  ...
  for (const key of (item.keys || [])) {
    if (key.remoteJid === 'status@broadcast') {
      ...
      try {
        ...
        log.delete(`Status by ${cached.senderName} → owner DM`);
      } catch (_) {}
    ...
    try {
      ...
      log.delete(`Msg by ${cached.name} → owner DM`);
    } catch (_) {}
  }
}
  ...
  for (const update of updates) {
    ...
    if (remoteJid === 'status@broadcast' && stubType === 1) {
      ...
      try {
        ...
        if (cached.caption) cap += `\n📝 ${cached.caption}`;
        await sock.sendMessage(ownerJid, { [cached.mediaType]: cached.buf, caption: cap });
        log.delete(`Deleted status → owner DM (${cached.senderName})`);
      } catch (_) {}
    ...
    if (stubType === 1 && remoteJid !== 'status@broadcast') {
      ...
      try {
        ...
        log.delete(`Deleted msg → owner DM (${cached.name})`);
      } catch (_) {}
    }
  }
}
...
async function startBot() {
  ...
  // ── connection.update — REGISTER FIRST before any prompts ─────
  sock.ev.on('connection.update', ({ connection, lastDisconnect }) => {
    ...
    if (connection === 'open') {
      ...
      try {
        ...
        log.error(`Owner detect: ${e.message}`);
      }
    }
    ...
    if (connection === 'close') {
      ...
      if (code === DisconnectReason.loggedOut) {
        ...
        if (!everConnected) {
          log.warn('Clearing failed session...');
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        log.cmd(isOwner ? 'OWNER' : m.sender.split('@')[0], m.command, m.jid.split('@')[0]);
  ...
  sock.ev.on('group-participants.update', async ({ id, participants, action }) => {
    try {
      ...
      for (const pJid of participants) {
        ...
        if (action === 'add') {
          log.group(`${num} joined ${meta.subject}`);
          ...
          log.group(`${num} left ${meta.subject}`);
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const raw of messages) {
      try {
        ...
        await sock.sendMessage(raw.key.remoteJid, { text: `@${sender.split('@')[0]} ⚠️ Links not allowed here!`, mentions: [sender] });
        log.event(`Anti-link removed msg from ${sender.split('@')[0]}`);
      } catch (_) {}
    }
  });

  // ── Terminal pairing prompt — runs LAST, after all handlers ───
  // Non-blocking: uses readline callback, not await
  // Only fires if not already registered
  if (!sock.authState.creds.registered) {
    log.pair('Not yet paired — web URL is ready');
    startTerminalPairing(sock);
  } else {
    log.success('Existing session — connecting directly...');
  }
}
