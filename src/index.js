...
function askPhoneInTerminal() {
  return new Promise((resolve) => {
    ...
    rl.question(chalk.bold.hex('#00ffc8')('  ➤ Phone number: '), (input) => {
      rl.close();
      resolve(input.trim().replace(/[^0-9]/g, ''));
    });
  });
}
...
async function startBot() {
  ...
  // ── Socket — NO browser override (same as working version) ────
  ...
  // ── Start web pairing server once ─────────────────────────────
  if (!pairingServerStarted) {
    pairingServerStarted = true;
    startPairingServer(config.PORT);
  }

  // ── Terminal pairing — only if not yet registered ─────────────
  // requestPairingCode is called ONCE here.
  // The code is cached so the web form returns the SAME code,
  // not a new one. This prevents double-call corruption.
  if (!sock.authState.creds.registered) {
    log.pair('Not yet paired.');
    log.pair('Option 1: Visit your web URL to pair');
    log.pair('Option 2: Enter your number below in the terminal');

    const termNumber = await askPhoneInTerminal();

    if (termNumber && termNumber.length >= 7) {
      // User entered a number in terminal — generate code here
      log.pair(`Requesting code for: ${termNumber}...`);
      try {
        await new Promise(r => setTimeout(r, 3000));
        const code = await sock.requestPairingCode(termNumber);
        config.OWNER = termNumber;
        // ── Cache the code so web /pair returns THIS same code ────
        // This means requestPairingCode is only EVER called once
        setCachedCode(code);
        console.log('');
        ...
        log.warn('Use the web URL instead to pair manually');
      }
    } else {
      // User skipped terminal — web URL is the only option
  ...
  // ── Main message event — notify AND append ────────────────────
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        log.cmd(
          isOwner ? 'OWNER' : m.sender.split('@')[0],
          m.command,
          m.jid.split('@')[0]
        );
