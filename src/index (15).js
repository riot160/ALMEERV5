  ...
  if (!sock.authState.creds.registered) {
    ...
    if (envNumber) {
      ...
      try {
        ...
        // Cache the code so web /pair returns same code, not a new one
        setCachedCode(code);
      ...
      log.pair('No OWNER_NUMBER in env — enter number in terminal or use web URL');
      log.info(`Web pairing available at your deployment URL`);
      ...
      if (termNumber && termNumber.length >= 7) {
        ...
        try {
          ...
          // Cache so web returns same code
          setCachedCode(code);
          console.log(chalk.bold.hex('#39ff14')('\n  ╔══════════════════════════════════════════════╗'));
          console.log(chalk.bold.hex('#39ff14')('  ║') + chalk.bold.white('   🔑 PAIRING CODE  ') + chalk.bold.hex('#00ffc8')(code) + chalk.bold.hex('#39ff14')('         ║'));
          console.log(chalk.bold.hex('#39ff14')('  ╚══════════════════════════════════════════════╝'));
          ...
          log.info(`Use the web URL to pair manually`);
        }
      } else {
        log.warn('No number entered — use the web URL to pair manually');
      }
    }
  } else {
