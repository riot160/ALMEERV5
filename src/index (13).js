...
import pino from 'pino'; import chalk from 'chalk';
...
export const settings = {
  ...
  alwaysOnline:     false,
  statusEmoji:      '🔥',
  statusDelay:      1000,   // delay (ms) before view/react to avoid rate limiting
};

export const msgCache    = new Map();
export const statusCache = new Map();

let retryCount = 0, pluginsLoaded = false, pairingServerStarted = false, everConnected = false;
const MAX_RETRIES = 5;

function printBanner() {
  try { console.log(gradient.rainbow(figlet.textSync('ALMEERV5', { font: 'ANSI Shadow' }))); }
...
async function statusHandler(sock, msg) {
  ...
  // ── AUTO VIEW STATUS ──────────────────────────────────────────
  // RIOT2 exact: sendReadReceipt(senderJid, null, [msgId])
  // This sends the read receipt to the SENDER'S JID — WhatsApp
  // checks this to mark the status as "viewed" on their end.
  if (settings.autoViewStatus && sender) {
    // Configurable delay to avoid WhatsApp rate limiting
    await new Promise(r => setTimeout(r, settings.statusDelay ?? 1000));
    try {
      await sock.sendReadReceipt(sender, null, [msg.key.id]);
      console.log(chalk.green(`  👁️  Viewed status: ${senderName} (+${senderNum})`));
    } catch (e1) {
      // Fallback: readMessages with explicit key
      try {
  ...
  // ── AUTO REACT STATUS ─────────────────────────────────────────
  // RIOT2 exact: sendMessage to SENDER with react key pointing to
  // status@broadcast. This is how WhatsApp registers a status reaction.
  if (settings.autoReactStatus && sender) {
    ...
    try {
      ...
      console.log(chalk.green(`  ${emoji}  Reacted to status: ${senderName}`));
    } catch (e) {
  ...
  if (settings.antiDeleteStatus && hasMedia) {
    try {
      const stream = await downloadContentFromMessage(
        msg.message[mediaKey], mediaType
      );
      const chunks = [];
...
// ── messages.update handler (stub type 1 — RIOT2 pattern) ────────────────────
  ...
  for (const update of updates) {
    ...
    if (remoteJid === 'status@broadcast' && stubType === 1) {
      ...
      try {
        ...
        if (cached.caption) cap += `\n📝 ${cached.caption}`;
        await sock.sendMessage(ownerJid, { [cached.mediaType]: cached.buf, caption: cap });
        console.log(chalk.dim(`  🗑️  Deleted status → owner DM from ${cached.senderName}`));
      } catch (_) {}
...
async function startBot() {
  ...
  if (!pluginsLoaded) { await loadPlugins(); pluginsLoaded = true; }

  // NO browser override — exact V4 config
  ...
  sock.ev.on('connection.update', ({ connection, lastDisconnect }) => {
    if (connection === 'connecting') console.log(chalk.dim('  Connecting to WhatsApp...'));
        ...
        if (myNumber) {
          ...
          sock.sendMessage(`${myNumber}@s.whatsapp.net`, {
            text:
              ...
              `✅ Bot connected!\n` +
              `📱 *Number:* ${myNumber}\n` +
              `👑 *Owner:* ${myNumber} _(auto-detected)_\n` +
              `🟢 *Node:* ${process.version}\n\n` +
              `_Type ${config.PREFIX}menu for commands_ 🚀`,
          }).catch(() => {});
        }
      } catch (e) { console.error(chalk.red('  Owner detect:'), e.message); }
    }
  ...
  // ── MAIN message handler — accepts notify AND append ──────────
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // Route status to status handler
        ...
        // Anti-delete DM via protocolMessage (RIOT2 pattern)
        ...
        // Only process commands on notify
  ...
  // Delete events — two listeners for full coverage
