...
function buildMenu(pushName, sock) {
  ...
  const cats = {};
  const icons = {
    general:'🔰', media:'🎵', ai:'🤖', fun:'🎉',
    utility:'🔧', settings:'⚙️', group:'👥', owner:'👑', misc:'📦',
  };
  ...
  const labels = {
    owner:'👑 OWNER', settings:'⚙️ SETTINGS', group:'👥 GROUP',
    ai:'🤖 AI', media:'⬇️ MEDIA', utility:'🔧 UTILITY',
    fun:'🎉 FUN', general:'🔰 GENERAL', misc:'📦 MISC',
  };
...
export default [
  {
    ...
    async run({ sock, m, args }) {
      ...
      if (sub) {
        ...
        if (!cmds.length) return m.reply(`❌ Unknown category: *${sub}*`);
